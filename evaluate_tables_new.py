...
try:
    # UPDATED FOR MARKER V1.0+
    from marker.converters.pdf import PdfConverter
    from marker.models import create_model_dict
    from marker.output import text_from_rendered
    MARKER_AVAILABLE = True
except Exception as e:
    print(f"\n Marker not found MARKER IMPORT ERROR: {e}\n")
...
class MarkerParser(TableParser):
    ...
    def __init__(self):
        ...
        print("Loading Marker models (this will download ~2GB on first run)...")
    ...
    def parse_pdf(self, pdf_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            # Convert PDF to markdown (New API)
            rendered = self.converter(pdf_path)
            full_text, _, _ = text_from_rendered(rendered)

            # Marker outputs the whole doc as markdown; extract tables
            ...
            for table_num, table_md in enumerate(table_blocks, 1):
                ...
                if df is not None and not df.empty:
                    tables.append({
                        'parser': self.name,
                        # Marker merges pages into one stream, so we default to page 1
                        'page': 1,
                        'table_num': table_num,
                        'data': df,
                        'raw_data': table_md
    ...
    def _initialize_parsers(self) -> List[TableParser]:
        ...
        if LLAMA_PARSE_AVAILABLE and os.getenv("LLAMA_CLOUD_API_KEY"):
            try:
                ...
                print(f" Could not initialize LlamaParse: {e}")
